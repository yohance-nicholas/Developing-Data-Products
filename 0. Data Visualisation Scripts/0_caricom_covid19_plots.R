...
# Mortality Rate among OECS Member States
caricom_tidycovid19 %>%
  filter(oecs == "OECS Member State",
         date >= as.Date("2020-03-07")
  ) %>%
  ggplot(aes(x = date, y = mortality_rate, col = country)) +
  geom_line() +
  labs(x = "Date",
       y = "Mortality Rate",
       title = "COVID-19 Mortality Rate among OECS Member States",
       caption = "Source: 2019 Novel Coronavirus COVID-19 (2019-nCoV) Data Repository by Johns Hopkins CSSE
       Prepared by: Yohance Nicholas")
ggsave(fname(), path = "2. Plots", dpi = 600)

# Mortality Rate among Worst Affected CARICOM Member States
caricom_tidycovid19 %>%
  filter(iso3c %in% top_6_mortality,
         date >= as.Date("2020-03-07")) %>%
  ggplot(aes(x = date, y = mortality_rate, col = country)) +
  geom_line() +
  labs(x = "Date",
       y = "Mortality Rate",
       title = "COVID-19 Mortality Rate among Worst Affected CARICOM Member States",
       caption = "Source: 2019 Novel Coronavirus COVID-19 (2019-nCoV) Data Repository by Johns Hopkins CSSE
       Prepared by: Yohance Nicholas")
ggsave(fname(), path = "2. Plots", dpi = 600)

# Recovery Rate among Worst Affected CARICOM Member States
caricom_tidycovid19 %>%
  filter(iso3c %in% top_6,
         date >= as.Date("2020-03-07")) %>%
  ggplot(aes(x = date, y = recovery_rate, col = country)) +
  geom_line() +
  labs(x = "Date",
       y = "Recovery Rate",
       title = "COVID-19 Recovery Rate among Worst Affected CARICOM Member States",
       caption = "Source: 2019 Novel Coronavirus COVID-19 (2019-nCoV) Data Repository by Johns Hopkins CSSE
       Prepared by: Yohance Nicholas")
ggsave(fname(), path = "2. Plots", dpi = 600)
...
caricom_tidycovid19 %>% mutate(cases_logratio = difference(log(confirmed))) %>%
  ...
  labs(x = "Date",
      y = "Log-Ratio of Confirmed Cases",
      title = "Log-Ratios of Cumulative COVID-19 Cases among Top 6 Worst Affected CARICOM Member States",
      caption = "Source: 2019 Novel Coronavirus COVID-19 (2019-nCoV) Data Repository by Johns Hopkins CSSE
       Prepared by: Yohance Nicholas") +
...
# Slider for CARICOM Economies 
slider_df <- caricom_tidycovid19 %>%
  filter(date >= "2020-03-10") %>% 
  select(date, iso3c, mortality_rate) %>% 
  spread(iso3c, mortality_rate) 

caricom_slider <- plot_ly(slider_df, x = ~date)
caricom_slider <- caricom_slider %>% add_lines(y = ~ATG, name = "Antigua and Barbuda")
caricom_slider <- caricom_slider %>% add_lines(y = ~BHS, name = "The Bahamas")
caricom_slider <- caricom_slider %>% add_lines(y = ~BLZ, name = "Belize")
caricom_slider <- caricom_slider %>% add_lines(y = ~BRB, name = "Barbados")
caricom_slider <- caricom_slider %>% add_lines(y = ~DMA, name = "Dominica")
caricom_slider <- caricom_slider %>% add_lines(y = ~GRD, name = "Grenada")
caricom_slider <- caricom_slider %>% add_lines(y = ~GUY, name = "Guyana")
caricom_slider <- caricom_slider %>% add_lines(y = ~HTI, name = "Haiti")
caricom_slider <- caricom_slider %>% add_lines(y = ~KNA, name = "St. Kitts and Nevis")
caricom_slider <- caricom_slider %>% add_lines(y = ~LCA, name = "St. Lucia")
caricom_slider <- caricom_slider %>% add_lines(y = ~VCT, name = "St. Vincent and the Grenadines")
caricom_slider <- caricom_slider %>% add_lines(y = ~SUR, name = "Suriname")
caricom_slider <- caricom_slider %>% add_lines(y = ~TTO, name = "Trinidad and Tobago")
caricom_slider <- caricom_slider %>% layout(
  title = "Confirmed Cases per 100,000 persons",
  xaxis = list(
    rangeselector = list(
      buttons = list(
        list(
